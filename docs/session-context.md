...
- **动功能之前先读[路线图](https://github.com/MR1ZK/tacz-z-tweaks/issues/1)**：一次 wayfinder 寻路留下的决策地图（1 张地图 + 16 张票），每个待做的功能都有一张票承载它的决策、取舍与依赖。开工前的规划稿在 `docs/refit-plus-plan.md`（已标注哪几节作废），三份调研在 `docs/research/`
...
- 闲置卡片第三列预留了但没放数据。**候选与顺序已有结论**（`docs/research/client-gun-params.md`）：开火模式 → 弹匣容量 → 射速 → 弹速 → 穿透 → 开镜时间；后坐力**不该放**（样条曲线，非标量）。归属票：整枪对比面板
- 配置屏没有"恢复默认"按钮
