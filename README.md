...
| 候选列表 | 只列当前槽位装得上的配件。不在背包里的压暗加红点，点安装会提示"缺少配件" |
| 概览态 | 没选槽位时不画候选框，那块区域整块渲染 TACZ 自己的枪械 tooltip（名字、描述、口径、枪种、伤害、护甲穿透、爆头伤害、移动速度、枪械包名，跟物品栏里看到的完全一致），其余区域交还给 3D 预览的拖拽和缩放 |
| 搜索与双击 | 候选框底部有搜索框，按显示名过滤，切槽位自动清空；双击候选行直接装上，省掉"点配件再点安装" |
